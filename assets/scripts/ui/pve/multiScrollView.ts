...
export class MultiScrollView extends Component {
    ...
    /**
     * 构建关卡列表：CSV中的关卡 + 动态生成的关卡
     * 跟原来固定50关完全一样的逻辑，只是总数从50变成动态计算
     * 原来50关 = [1, 22, 22, 5]，(50-1)%22=5 → 最后一页5关+云
     */
    buildLevelList () {
        var csvLevels = utils.objectToArray(localConfig.instance.getTable('level'));
        var currentLevel = playerData.instance.getCurrentLevel();
        var maxLevelId = Number(currentLevel.ID);

        // 跟原来50关一样：总数 = 当前关卡所在页填满 + 下一页放5关（跟原来最后一页一样）
        var afterFirst = maxLevelId - 1; // 去掉第1关
        var fullPages = Math.ceil(afterFirst / 22); // 当前关卡需要几个满页
        var totalLevels = 1 + fullPages * 22 + 5; // 1(首页) + 满页 + 5(最后一页，跟原来一样)

        var result = csvLevels.slice();

        var csvMaxId = csvLevels.length > 0 ? Number(csvLevels[csvLevels.length - 1].ID) : 0;
        for (var i = csvMaxId + 1; i <= totalLevels; i++) {
            result.push(playerData.instance.generateLevelInfo(i));
        }
